...
public class AuthorizationCommands : CommandGroup {
    ...
    public AuthorizationCommands(AuthorizationService authorization, ICommandContext commandContext, FeedbackService feedback) {
    ...
    public async Task<IResult> GrantAsync(
        ...
        [Description("The role to grant the permission to. Use @everyone to set the default")] IRole role
    ) {
        IResult grant;
        string message;
    ...
    public async Task<IResult> DenyAsync(
        [Description("The permission to deny")] Permission permission,
        [Description("The role to deny the permission to. Use @everyone to set the default")] IRole role
    ) {
        IResult deny;
        string message;
        if(role.ID == _commandContext.GuildID.Value) {
    ...
    public async Task<IResult> ResetAsync(
        [Description("The permission to reset")] Permission permission,
        [Description("The role to reset the permission entry from. Use @everyone to set the default")] IRole role
    ) {
        IResult reset;
        string message;
        if(role.ID == _commandContext.GuildID.Value) {
